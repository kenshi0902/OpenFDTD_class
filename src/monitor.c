...
// output files
static void monitor3_(FILE *fp, int ev, int geom)
{
	fprintf(fp, "=== output files ===\n");
	fprintf(fp, "%s", FN_log);
	if ((geom == 0) || (geom == 1)) {
		fprintf(fp, ", %s", (ev ? FN_geom3d_1 : FN_geom3d_0));
	}
	if      (runMode == 0) {
...
void monitor3(FILE *fp, int ev, int geom)
{
	monitor3_(fp,     ev, geom);
	monitor3_(stdout, ev, geom);
}
