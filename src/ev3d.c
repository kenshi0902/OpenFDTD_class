...
-3 I X Y Z H R G B N F                        draw a string (3D, evgl.exe only)
characters                                    string
-4 I H R G B N F                              draw a title (2D, ev3d.exe only)
...
static void ev3d_end_html(FILE *fp, const char fn[])
{
	...
	fprintf(fp, "		context.font = h + \"px \" + Title[ipage][i][5];\n");
	...
	for (int page = 0; page <= ev3d_page; page++) {
		...
		for (int n = 0; n < ev3d_ndata[page]; n++) {
			...
			if (ptr->idx > 0) {
				...
				fprintf(fp, " %g %u %u %u %d %d\n", ptr->hgt, ptr->rgb[0], ptr->rgb[1], ptr->rgb[2], (int)strlen(ptr->str), ptr->fontname);
				fprintf(fp, "%s\n", ptr->str);
			}
			else if (ptr->idx == -4) {
				// title
...
// set fontname
void ev3d_setFont(const char font[])
{
	if      (!strcmp(font, "sansserif")) {
		ev3d_fontname = 0;
	}
	else if (!strcmp(font, "serif")) {
		ev3d_fontname = 1;
	}
	else if (!strcmp(font, "monospace")) {
		ev3d_fontname = 2;
	}
}
