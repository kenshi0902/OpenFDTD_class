...
char ev2d_fontname[BUFSIZ] = "sansserif";  // "sansserif", "serif", "monospace"
int ev2d_page = -1;
int ev2d_width = 0;
int ev2d_height = 0;
	...
	for (int page = 0; page <= ev2d_page; page++) {
		...
		for (int n = 0; n < ev2d_ndata[page]; n++) {
			...
			if      (ptr->idx == 2) {
				...
				fprintf(fp, "%d %d %d %s\n", ptr->xvt[0], ptr->yvt[0], ptr->hgt, ptr->fontname);
				fprintf(fp, "%s\n", ptr->str);
			}
			else if ((ptr->idx == 21) || (ptr->idx == 22)) {
				fprintf(fp, "%d %d %d %d\n", ptr->xvt[0], ptr->yvt[0], ptr->xvt[1], ptr->yvt[1]);
...
void ev2d_drawString(double x, double y, double h, const char str[])
{
	...
	// font
	ev2d_data[ev2d_page][ndata].fontname = (char *)malloc((strlen(ev2d_fontname) + 1) * sizeof(char));
	strcpy(ev2d_data[ev2d_page][ndata].fontname, ev2d_fontname);
...
// set fontname
void ev2d_setFont(const char fontname[])
{
	strcpy(ev2d_fontname, fontname);
}
